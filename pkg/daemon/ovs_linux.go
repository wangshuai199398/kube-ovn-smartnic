...
import (
	...
	"github.com/kubeovn/kube-ovn/pkg/yusur"
)
...
func (csh cniServerHandler) configureNic(podName, podNamespace, provider, netns, containerID, vfDriver, ifName, mac string, mtu int, ip, gateway string, isDefaultRoute, detectIPConflict bool, routes []request.Route, _, _ []string, ingress, egress, deviceID, nicType, latency, limit, loss, jitter string, gwCheckMode int, u2oInterconnectionIP, oldPodName string) ([]request.Route, error) {
	...
	if deviceID != "" {
		...
		if pod.Labels[util.KubeVirtVdpa] == "true" {
		...
		pod.Annotations[util.VhostUserVdpaPciAnnotation] = deviceID
		var patch []byte
	...
	finalRoutes, err := csh.configureContainerNic(podName, podNamespace, containerNicName, ifName, ip, gateway, isDefaultRoute, detectIPConflict, routes, macAddr, podNS, mtu, nicType, gwCheckMode, u2oInterconnectionIP, isVdpaVm, deviceID)
	if err != nil {
		klog.Error(err)
		return nil, err
	}
...
func (csh cniServerHandler) releaseVf(podName, podNamespace, podNetns, ifName, nicType, provider, deviceID string, isVdpaVm bool) error {
	...
	if isVdpaVm {
		if err := DriverVfioToYsK2nic(deviceID); err != nil {
			return fmt.Errorf("[KubevirtMachinie] failed bind/unbind driver. err: %v", err)
		}
		klog.Infof("change device %s driver from vfio-pci to ysnic_k2", deviceID)
	} else {
...
func (csh cniServerHandler) deleteNic(podName, podNamespace, containerID, netns, deviceID, ifName, nicType, provider string, isVdpaVm bool) error {
	if err := csh.releaseVf(podName, podNamespace, netns, ifName, nicType, provider, deviceID, isVdpaVm); err != nil {
	...
	var nicName string
	if yusur.IsYusurSmartNic(deviceID) {
		pfPci, err := yusur.GetPfPciFromVfPci(deviceID)
		if err != nil {
			return fmt.Errorf("failed to get pf pci %w, %s", err, deviceID)
		}

		pfIndex, err := yusur.GetYsk2PfIndexByPciAddress(pfPci)
		if err != nil {
			return fmt.Errorf("failed to get pf index %w, %s", err, deviceID)
		}

		vfIndex, err := yusur.GetYsk2VfIndexByPciAddress(deviceID)
		if err != nil {
			return fmt.Errorf("failed to get vf index %w, %s", err, deviceID)
		}

		nicName = yusur.GetYsk2VfRepresentor(pfIndex, vfIndex)
	} else {
...
func (csh cniServerHandler) configureContainerNic(podName, podNamespace, nicName, ifName, ipAddr, gateway string, isDefaultRoute, detectIPConflict bool, routes []request.Route, macAddr net.HardwareAddr, netns ns.NetNS, mtu int, nicType string, gwCheckMode int, u2oInterconnectionIP string, isVdpaVm bool, deviceID string) ([]request.Route, error) {
	...
	err = ns.WithNetNSPath(netns.Path(), func(_ ns.NetNS) error {
		...
		if isVdpaVm {
			if err := DriverYsK2nicToVfio(deviceID); err != nil {
				return fmt.Errorf("[KubevirtMachinie] failed bind/unbind driver. err: %v", err)
			}
			klog.V(5).Infof("[KubevirtMachinie] change device %s driver from ysnic_k2 to vfio-pci", deviceID)
			return nil
		}
...
func setupSriovInterface(containerID, deviceID, vfDriver, ifName string, mtu int, mac string) (string, string, string, int, error) {
	...
	if yusur.IsYusurSmartNic(deviceID) {
		...
		rep := yusur.GetYsk2VfRepresentor(pfIndex, vfIndex)
...
func (csh cniServerHandler) configureNicWithInternalPort(podName, podNamespace, provider, netns, containerID, ifName, mac string, mtu int, ip, gateway string, isDefaultRoute, detectIPConflict bool, routes []request.Route, _, _ []string, ingress, egress, _, nicType, latency, limit, loss, jitter string, gwCheckMode int, u2oInterconnectionIP string) (string, []request.Route, error) {
	...
	routes, err = csh.configureContainerNic(podName, podNamespace, containerNicName, ifName, ip, gateway, isDefaultRoute, detectIPConflict, routes, macAddr, podNS, mtu, nicType, gwCheckMode, u2oInterconnectionIP, false, "")
...
func setStringSysctl(sysctl string, newVal string) error {
	return os.WriteFile(sysctl, []byte(newVal), 0o640)
}

func DriverYsK2nicToVfio(pci string) error {
	var err error
	unbind := fmt.Sprintf("%s/unbind", util.Ysk2DriverPci)
	bind := fmt.Sprintf("%s/bind", util.VfioDriverPci)
	newId := fmt.Sprintf("%s/new_id", util.VfioDriverPci)

	_, err = os.Stat(newId)
	if err != nil {
		return err
	}
	if err = setStringSysctl(newId, util.YusurId); err != nil {
		klog.Warningf("[KubevirtMachinie] Write new_id err: %v", err)
	}
	_, err = os.Stat(unbind)
	if err != nil {
		return err
	}
	if err = setStringSysctl(unbind, pci); err != nil {
		klog.Warningf("[KubevirtMachinie] Write unbind err: %v", err)
	}
	_, err = os.Stat(bind)
	if err != nil {
		return err
	}
	if err = setStringSysctl(bind, pci); err != nil {
		klog.Warningf("[KubevirtMachinie] Write bind err: %v", err)
	}

	return nil
}

func DriverVfioToYsK2nic(pci string) error {
	var err error
	bind := fmt.Sprintf("%s/bind", util.Ysk2DriverPci)
	unbind := fmt.Sprintf("%s/unbind", util.VfioDriverPci)
